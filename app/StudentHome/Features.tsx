...
const Features = () => {
  return (
    <section id="features"  className="py-20 bg-white" style={{ background: 'linear-linear(135deg, #f0f4f8 0%, #ffffff 100%)' }}>
        ...
        <div data-aos="fade-up" className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          {features.map((feature, index) => (
            <div key={index} className=" bg-white border-2 border-transparent rounded-2xl p-8 shadow-lg hover:shadow-2xl hover:-translate-y-2 transition-all duration-300 group cursor-pointer" style={{ transitionDelay: `${index * 100}ms` }}>
              <div className="w-14 h-14 bg-linear-to-br from-[#1887A1] to-[#0D4C5B] rounded-xl flex items-center justify-center mb-6 group-hover:animate-bounce-slow transition-transform duration-300">

...
export default function MyStudentsSection({ myStudents }: MyStudentsSectionProps) {
    return (
            ...
            <div className="bg-white rounded-xl p-4 lg:p-6 shadow-sm border border-gray-100">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4 lg:gap-6">
                    {myStudents.map((student) => (
                        <div key={student.id} className="p-4 lg:p-6 border border-gray-200 rounded-lg hover:shadow-lg hover:border-blue-300 transition-all duration-300 cursor-pointer transform hover:scale-105">
                            <div className="flex items-center space-x-3 mb-4">
                                ...
                                <div>
                                    ...
                                    <p className="text-sm text-gray-600">{student.subject} • {student.level}</p>
                                </div>
                            </div>
                            ...
                            <div className="flex items-center justify-between">
                                <div>
                                    ...
                                    <span className={`text-xs px-2 py-1 rounded-full mt-1 inline-block ${student.status === 'Active' ? 'bg-green-100 text-green-600' : 'bg-gray-100 text-gray-600'
                                        }`}>
                                        {student.status}
                                    </span>
                                </div>
                                <button className="text-sm font-medium px-3 py-1 rounded-lg transition-all duration-200 hover:bg-blue-50" style={{ color: '#1887A1' }}>
                                    View Details
                                </button>
                            </div>
                        </div>
